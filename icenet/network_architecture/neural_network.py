...
from icenet.utilities.random_stuff import no_op
from icenet.utilities.to_torch import to_cuda, maybe_to_torch
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            if m == 0:
                pred = self.inference_apply_nonlin(self(x)[0])  # take only the prediction tensor

                #pred = self.inference_apply_nonlin(self(x))
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                logits = self(torch.flip(x, (4, )))
                if isinstance(logits, tuple):
                   logits = logits[0]  # Take only the first item from the tuple
            ...
            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                logits = self(torch.flip(x, (4,3,2 )))
                if isinstance(logits, tuple):
                   logits = logits[0]  # Take only the first item from the tuple
                pred = self.inference_apply_nonlin(logits)
