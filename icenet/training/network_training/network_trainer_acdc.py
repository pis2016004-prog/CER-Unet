...
class NetworkTrainer_acdc(object):
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            with autocast():
                ...
                #l = l + 0.01 * k_loss  # KL loss, can be removed if not used
    ...
    #     if self.fp16:
    #         with autocast():
    #             logits, kl_loss = self.network(data)
    #             del data
    #             l = self.loss(logits, target)
    #             l_total = l + 0.01 * kl_loss

    #         if do_backprop:
    #             self.amp_grad_scaler.scale(l_total).backward()
    #             self.amp_grad_scaler.step(self.optimizer)
    #             self.amp_grad_scaler.update()
    #     else:
    #         logits, kl_loss = self.network(data)
